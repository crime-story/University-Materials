...
vector<int> Graf::cicluEulerian() {
    ...
    for (int i = 1; i <= nrNoduri; i++)
        if (listaDeAdiacenta[i].size() % 2 == 1) { // daca un nod nu are gradul par inseamna ca nu exista un ciclu eulerian
            ciclu.push_back(-1);
            return ciclu;
        }
...
int Graf::cicluHamiltonian() {
    int cost = INFINIT;
    // costul initial e infinit pentru toata lumea
    vector<vector<int>> C(1 << nrNoduri, vector<int>(nrNoduri + 1, INFINIT));
    C[1][0] = 0; // 2 ^ 0 = 1, se termina in 0, deci e costul initial este 0
    for (int i = 0; i < (1 << nrNoduri); i++) // pentru toate configuratiile de a fi facut ciclul hamiltonian pana acum
        for (int j = 0; j < nrNoduri; j++) // pentru toate nodurile pe le-as putea avea pe ultima pozitie
            if (i & (1 << j)) // daca avem o configuratie valida, daca 1 shiftat de j & i este 1
                for (auto &k: listaDeAdiacenta[j]) { // iau toti vecinii
                    int vecin = k.destinatie;
                    int costVecin = k.cost;
                    if (i & (1 << vecin))
                        // cea mai buna configuratie este minimul dintre mine si ce ar fi fost daca nu eram eu pe
                        // ultima pozitie si veneam din vecinul curent al meu
                        // configuratia 23 daca scot nodul 4 obtinem 19 si eu stiu de la ce vecin vin
                        C[i][j] = min(C[i][j], C[i ^ (1 << j)][vecin] + costVecin);
                }
    for (auto &i: listaDeAdiacenta[0]) // solutia va fi minimul dintre toate configuratiile care se pot intoarce la inceput
