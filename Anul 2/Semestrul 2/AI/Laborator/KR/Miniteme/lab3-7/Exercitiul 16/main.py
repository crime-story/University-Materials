...
class Graph:  # graful problemei
    def __init__(self, nume_fisier):
        ...
        self.start = (self.__class__.N, self.__class__.N, self.__class__.N, self.__class__.N, 0, 0, 0, 0, 1)  # informatia nodului de start
                        # canibali total mal stang(viz+neviz), misionari total stang, canibali neviz stang, mis neviz stang, canibal total drept, misionar total drept, canibali neviz drept, mis neviz drept, poz barca
    # self.scopuri=[(0,0,0)]

    def testeaza_scop(self, nodCurent):
        return nodCurent.info[0] == nodCurent.info[1] == self.__class__.N and nodCurent.info[2] == nodCurent.info[3] == nodCurent.info[4] == nodCurent.info[5] == nodCurent.info[6] == nodCurent.info[7] == 0 and nodCurent.info[8] == 1
    ...
    def genereazaSuccesori(self, nodCurent, tip_euristica="euristica banala"):
        ...
        barca = nodCurent.info[8]
        if barca == 1:  # malul barcii este cel initial
            canibali_mal_curent_nevizitat = nodCurent.info[0]
            misionari_mal_curent_nevizitat = nodCurent.info[1]
            canibali_mal_opus_nevizitat = nodCurent.info[4]
            misionari_mal_opus_nevizitat = nodCurent.info[5]

            canMalCurent = nodCurent.info[0] + nodCurent.info[2]
            misMalCurent = nodCurent.info[1] + nodCurent.info[3]
            canMalOpus = nodCurent.info[4] + nodCurent.info[6]
            misMalOpus = nodCurent.info[5] + nodCurent.info[7]

            maxMisionariBarca = min(Graph.M, misionari_mal_curent_nevizitat)
        else:  # barca==0 adica malul final
            canibali_mal_opus_nevizitat = nodCurent.info[0]
            misionari_mal_opus_nevizitat = nodCurent.info[1]
            canibali_mal_curent_nevizitat = nodCurent.info[4]
            misionari_mal_curent_nevizitat = nodCurent.info[5]

            canMalOpus = nodCurent.info[0] + nodCurent.info[2]   # malul opus (barcii) este cel initial
            misMalOpus = nodCurent.info[1] + nodCurent.info[3]
            canMalCurent = nodCurent.info[4] + nodCurent.info[6]
            misMalCurent = nodCurent.info[5] + nodCurent.info[7]
            maxMisionariBarca = min(Graph.M, misMalCurent - misionari_mal_curent_nevizitat) # intoarcem doar misionarii care au vizitat
        for misBarca in range(maxMisionariBarca + 1):
            if barca == 1:
                if misBarca == 0:
                    maxCanibaliBarca = min(Graph.M, canibali_mal_curent_nevizitat)
                    minCanibaliBarca = 1
                else:
                    maxCanibaliBarca = min(Graph.M - misBarca, canibali_mal_curent_nevizitat, misBarca)
                    minCanibaliBarca = 0
            else:
                if misBarca == 0:
                    maxCanibaliBarca = min(Graph.M, canMalCurent - canibali_mal_curent_nevizitat)
                    minCanibaliBarca = 1
                else:
                    maxCanibaliBarca = min(Graph.M - misBarca, canMalCurent - canibali_mal_curent_nevizitat, misBarca)
                    minCanibaliBarca = 0
            for canBarca in range(minCanibaliBarca, maxCanibaliBarca + 1):
                ...
                canMalCurent_nou = canMalCurent - canBarca # scadem canibalii care pleaca
                misMalCurent_nou = misMalCurent - misBarca
                canMalOpus_nou = canMalOpus + canBarca # adunam la cel opus canibalii pt ca vin
                misMalOpus_nou = misMalOpus + misBarca
                if barca == 1: # malul stang
                    canMalCurent_nevizitat_nou = canibali_mal_curent_nevizitat - canBarca # pleaca din canibalii nevizitati
                    misMalCurent_nevizitat_nou = misionari_mal_curent_nevizitat - misBarca
                    canMalOpus_nevizitat_nou = canBarca # cei care erau deja pe acel mal vor devenii vizitati
                    misMalOpus_nevizitat_nou = misBarca
                else:
                    canMalCurent_nevizitat_nou = 0 # cand barca pleaca spre malul stang canibalii care au ramas pe malul drept se fac toti vizitati
                    misMalCurent_nevizitat_nou = 0
                    canMalOpus_nevizitat_nou = canibali_mal_opus_nevizitat # venim cu canibali vizitati deci nu se modifica numarul de canibali nevizitati
                    misMalOpus_nevizitat_nou = misionari_mal_opus_nevizitat


                if not test_conditie(misMalCurent_nou, canMalCurent_nou):
                    continue
                if not test_conditie(misMalOpus_nou, canMalOpus_nou):
                    continue
                if barca == 1:  # testul este pentru barca nodului curent (parinte) deci inainte de mutare
                    infoNodNou = (canMalCurent_nou, misMalCurent_nou, canMalCurent_nevizitat_nou, misMalCurent_nevizitat_nou, canMalOpus_nou, misMalOpus_nou, canMalOpus_nevizitat_nou, misMalOpus_nevizitat_nou, 0)
                else:
                    infoNodNou = (canMalOpus_nou, misMalOpus_nou, canMalOpus_nevizitat_nou, misMalOpus_nevizitat_nou, canMalCurent_nou, misMalCurent_nou, canMalCurent_nevizitat_nou, misMalCurent_nevizitat_nou, 1)
    ...
    def calculeaza_h(self, infoNod, tip_euristica="euristica banala"):
        if tip_euristica == "euristica banala":
            if not infoNod[0] == infoNod[1] == self.__class__.N and infoNod[2] == infoNod[3] == infoNod[4] == infoNod[5] == infoNod[6] == infoNod[7] == 0:
                return 1
            return 0
        else:
            # calculez cati oameni mai am de mutat si impart la nr de locuri in barca
            # totalOameniDeMutat=infoNod[0]+infoNod[1]
            return 2 * math.ceil((infoNod[0] + infoNod[1]) / (self.M - 1)) + (1 - infoNod[
                2]) - 1  # (1-infoNod[2]) vine de la faptul ca daca barca e pe malul final trebuie sa mai faca o trecere spre malul initial ca sa ii ia pe oameni, pe cand daca e deja pe malul initial, nu se mai aduna acel 1
