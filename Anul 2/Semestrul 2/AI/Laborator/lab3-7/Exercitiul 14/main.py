...
class Graph:  # graful problemei
    ...
    def testeaza_scop(self, nodCurent):
        for scop in self.scopuri:
            if not nodCurent.contineInDrum(scop):
                return False
        return True
            ...
            for j in range(nr_stive):  # j = indicele stivei pe care pun blocul
                ...
                if not nodCurent.contineInDrum(stive_n):
                    nod_nou = NodParcurgere(stive_n, nodCurent, cost=nodCurent.g + costMutareBloc,
                                            h=self.calculeaza_h(stive_n, nodCurent, tip_euristica))
                    listaSuccesori.append(nod_nou)

        return listaSuccesori

    # euristica banala
    def calculeaza_h(self, infoNod, nodCurent, tip_euristica="euristica banala"):
        if tip_euristica == "euristica banala":
        ...
        elif tip_euristica == "euristica admisibila robertto":
            # calculez cate blocuri nu sunt la locul fata de fiecare dintre starile scop, si apoi iau minimul dintre aceste valori
            euristici = []
            for (iScop, scop) in enumerate(self.scopuri):
                h = 0
            ...
            c = 0  # numar prin cate noduri finale inca nu am trecut
            if nodCurent is not None:
                for scop in self.scopuri:
                    if not nodCurent.contineInDrum(scop):
                        c += 1
            return min(euristici) + c
...
def a_star(gr, nrSolutiiCautate, tip_euristica):
    # in coada vom avea doar noduri de tip NodParcurgere (nodurile din arborele de parcurgere)
    c = [NodParcurgere(gr.start, None, 0, gr.calculeaza_h(gr.start, None, tip_euristica))]
...
nrSolutiiCautate = 3
tip_euristica = "euristica admisibila robertto"
nrSolutiiCautate = 3
cProfile.run("a_star(gr, nrSolutiiCautate, tip_euristica)")
